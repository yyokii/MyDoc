...
黒須正明氏のUX原論の定義が自分にしっくりくる（下記は一部自分の言葉に修正しているがほぼ同じ）

* 人工物とその利用者との関係において生じるものである
* 利用前の期待や予想、入手、そしてそれ以降の利用という時間軸の存在とその上で変動がある
* 対象となる人工物の知覚（近刺激として）に始まり、認知、記憶、感情の各システムに至るものであり、さらに時には運動系の反応としてあらわれる心理的プロセスである
* 個人的で主観的なものである

https://note.com/usagimaruma/n/n3a7bac7d9ed5 より

なので、

* 「UXが良い/悪い」という発言は利用者の像が固定されているときには可能だが、そうでない場合は個人の感想でありあたかも共通認識の像があると思わせてしまうので不適
* 「固定された利用者の像」はユーザーリサーチでそもそも得られないか、非常にコストがかかるものだと思う。ある程度抽象化した、具体的すぎないものでその像を定義するのがいいのかもしれない。
* 体験や結果の感想を言うのであれば誤解を招くUXという言葉を使うのではなく、「〜と感じた/思った」というような表現を使うようにする
...
* プラットフォームのルールに従う
iOSやmacOSでは『同意的アクションは右側に、否定的アクションは左側に配置する』と定義されているため、情報の配置ルールを意識するようにしましょう。
...
* 足す前に削る。足す前に「それは本当に必要か」を問う
* 多いほど良いのではなく、少ないほど伝わる・使いやすいことがある
* 1つの構図・1つのセクションには、だいたい1つの役割で足りる
* ノイズ・装飾・情報量を盛ることはデフォルトにしない
* 主役はユーザーとコンテンツ。UIやブランドは邪魔せず背景に退くこともある
* 既存のデザイン言語があるなら、それに乗る。独自の盛りはしない
...
* UI
  * 下記が定義されている
    ...
    * Ideal State（理想状態）
* 本当にそれは必要なUIなのか
  * ユーザーがそれを見れて嬉しくて、そのUIがあることで悪影響はないか
* 画面パターン
