...
* クロスプラットフォーム開発の価値は、単純に「コードを1回書けば済む」ことではない

  * iOS / Androidの実装コスト削減
  * 両プラットフォーム間の仕様同期コスト削減
  * QAコスト削減

    * 「この計算や判定についてはiOS側で十分に確認できているので、Android側では同等ケースの一部を間引く」といった判断が可能になる
  * エンジニアの学習・コンテキストスイッチ削減
  * 採用・チーム編成などの組織コスト削減

* 一方で、クロスプラットフォームでも消えないコストがある

  * App Store / Google Playそれぞれの運用
  * OS固有機能への対応
  * 実機・OSバージョン差分への対応
  * プラットフォーム固有のQA

* むしろクロスプラットフォームによって増えるコストもある

  * React Native / Flutterなど抽象化レイヤーそのものの保守
  * フレームワークのアップデート追従
  * ネイティブとの境界で発生する問題
  * フレームワーク固有の制約・デバッグ
...
* つまりクロスプラットフォームは、
  ...
  * **開発コストを消しているのではなく、別の場所に移している**
  * その移動先も含めて総コストを見る必要がある

## Coding Agentによって変わる前提

* Coding Agentによって、Swift / Kotlinで同じ機能を2回実装するコストは大きく下がる

* これまでクロスプラットフォームの大きなメリットだった

  * 「二重実装を避けられる」という価値が相対的に小さくなる

* その結果、

  * 抽象化レイヤーを持つよりSwift / Kotlinでそれぞれ素直にネイティブ実装するという選択が以前より現実的になる

* ShopifyがReact Nativeからネイティブへ戻る動きも、

  * 「React Nativeが失敗した」という単純な話ではなく
  * **AIによってネイティブ2本を維持するコスト構造が変わった**と捉える方が自然

## ただし、AIで全部解決するわけではない

* AIが特に下げるのは「コードを書くコスト」

* 一方で以下は依然として人間・仕組み側の課題
...
* Shopifyも単純にCoding Agentへコードを書かせているだけではなく、

  * テスト, Visual Review, Parity確認, 複数段階のレビューなど、AIが安全にコードを書ける開発基盤へ投資している
